...
module Stmt =
    let rec toStr indent stmt =
        ...
        | Define(_, t, x, e) -> sprintf "%s%s %s = %s" indent (CType.toStr t) x (Exp.toStr e)
        | Assign(_, x, e) -> sprintf "%s%s = %s" indent x (Exp.toStr e)
        | PtrUpdate(_, x, e) -> sprintf "%s*%s = %s" indent x (Exp.toStr e)
        | ArrUpdate(_, x, e1, e2) -> sprintf "%s%s[%s] = %s" indent x (Exp.toStr e1) (Exp.toStr e2)
        ...
        | If(_, e, s1, s2) ->
            ...
            sprintf "%sif (%s) [\n%s\n%s] else [\n%s\n%s]" indent expStr s1Str indent s2Str indent
        | While(_, e, s) ->
            let expStr = Exp.toStr e
