...
module Instr =
    ...
    let toStr instr =
        ...
        | BinOp(r, op, v1, v2) ->
            let leftStr = oprndToStr v1
            let binOpStr = binopToStr op
            let rightStr = oprndToStr v2
            sprintf "%s = %s %s %s" r leftStr binOpStr rightStr
...
module IRCode =
    let toStr (ir: IRCode) : string =
        ...
        let instrStr = List.map Instr.toStr instrs |> String.concat ",\n  "
        sprintf "%s(%s) : [\n  %s\n]" fname argStr instrStr
